...
export const contentfulLoader = (
  ...
  contentfulParams?: ContentfulImageParams,
) => {
    ...
    adjustedContentfulParams = {
      ...
      q: loaderProps.quality
        ...
        : contentfulParams.q
          ...
          : defaultQuality,
      w: loaderProps.width,
      h: contentfulParams.ar
        ? Math.round(loaderProps.width * parseAspectRatio(contentfulParams.ar))
        : undefined,
    };
  } else {
    adjustedContentfulParams = {
  ...
  return `${loaderProps.src}?${new URLSearchParams(
    adjustedContentfulParams as { [key: string]: string },
  ).toString()}`;
};

const parseAspectRatio = (ar: string) => {
  if (!ar.includes(':')) {
    throw new Error(
